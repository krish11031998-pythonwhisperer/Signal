...
class CircularProgressbar: UIView {
	...
	private func setupProgressBar() {
		...
		let circularPath = UIBezierPath(arcCenter: frame.center, radius: radius - 2,
	...
	public func configureChart(label: RenderableText? = nil, color: UIColor, _ val: CGFloat, visited: Bool) {
		if let validLabel = label {
            ...
            (String(format: "%.0f", val * 100) + "%").bodySmallRegular().render(target: innerText)
		}
	...
	public func animateValue(color: UIColor,_ val: CGFloat) {
		circularPathLayer?.animate(.circularProgress(to: val, duration: 0.75))
	}
}
